"use strict";

function _typeof(obj) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (obj) { return typeof obj; } : function (obj) { return obj && "function" == typeof Symbol && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }, _typeof(obj); }

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = SignModal;

var _react = _interopRequireWildcard(require("react"));

var _Box = _interopRequireDefault(require("@mui/material/Box"));

var _Modal = _interopRequireDefault(require("@mui/material/Modal"));

var _Stack = _interopRequireDefault(require("@mui/material/Stack"));

var _Close = _interopRequireDefault(require("@mui/icons-material/Close"));

var _Twitter = _interopRequireDefault(require("@mui/icons-material/Twitter"));

var _material = require("@mui/material");

var _utils = require("./utils/utils");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

function _getRequireWildcardCache(nodeInterop) { if (typeof WeakMap !== "function") return null; var cacheBabelInterop = new WeakMap(); var cacheNodeInterop = new WeakMap(); return (_getRequireWildcardCache = function _getRequireWildcardCache(nodeInterop) { return nodeInterop ? cacheNodeInterop : cacheBabelInterop; })(nodeInterop); }

function _interopRequireWildcard(obj, nodeInterop) { if (!nodeInterop && obj && obj.__esModule) { return obj; } if (obj === null || _typeof(obj) !== "object" && typeof obj !== "function") { return { "default": obj }; } var cache = _getRequireWildcardCache(nodeInterop); if (cache && cache.has(obj)) { return cache.get(obj); } var newObj = {}; var hasPropertyDescriptor = Object.defineProperty && Object.getOwnPropertyDescriptor; for (var key in obj) { if (key !== "default" && Object.prototype.hasOwnProperty.call(obj, key)) { var desc = hasPropertyDescriptor ? Object.getOwnPropertyDescriptor(obj, key) : null; if (desc && (desc.get || desc.set)) { Object.defineProperty(newObj, key, desc); } else { newObj[key] = obj[key]; } } } newObj["default"] = obj; if (cache) { cache.set(obj, newObj); } return newObj; }

function asyncGeneratorStep(gen, resolve, reject, _next, _throw, key, arg) { try { var info = gen[key](arg); var value = info.value; } catch (error) { reject(error); return; } if (info.done) { resolve(value); } else { Promise.resolve(value).then(_next, _throw); } }

function _asyncToGenerator(fn) { return function () { var self = this, args = arguments; return new Promise(function (resolve, reject) { var gen = fn.apply(self, args); function _next(value) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "next", value); } function _throw(err) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "throw", err); } _next(undefined); }); }; }

function _slicedToArray(arr, i) { return _arrayWithHoles(arr) || _iterableToArrayLimit(arr, i) || _unsupportedIterableToArray(arr, i) || _nonIterableRest(); }

function _nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }

function _unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return _arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return _arrayLikeToArray(o, minLen); }

function _arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; } return arr2; }

function _iterableToArrayLimit(arr, i) { var _i = arr == null ? null : typeof Symbol !== "undefined" && arr[Symbol.iterator] || arr["@@iterator"]; if (_i == null) return; var _arr = []; var _n = true; var _d = false; var _s, _e; try { for (_i = _i.call(arr); !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"] != null) _i["return"](); } finally { if (_d) throw _e; } } return _arr; }

function _arrayWithHoles(arr) { if (Array.isArray(arr)) return arr; }

function _objectDestructuringEmpty(obj) { if (obj == null) throw new TypeError("Cannot destructure undefined"); }

/* babel-plugin-inline-import './utils/metamask.svg' */
var metamask = "data:image/svg+xml;base64,";

/* babel-plugin-inline-import './utils/coinbase.png' */
var coinbase = "data:image/png;base64,";
var START_SIGN = 0;
var CONNECT_WALLET = 1;
var SIGN_MESSAGE = 2;
var VERIFY = 3;
var VERIFY_TWEET = 4;
var VERIFYING = 5;
var FINISH_SIGN = 6;
var SIGNING = 7;

var CloseButton = function CloseButton(_ref) {
  var handleClose = _ref.handleClose;
  return /*#__PURE__*/_react["default"].createElement(_material.IconButton, {
    onClick: handleClose,
    sx: {
      position: 'absolute',
      right: 15,
      top: 15,
      background: 'rgba(196, 196, 196, 0.2)',
      height: 48,
      width: 48,
      borderRadius: 2
    }
  }, /*#__PURE__*/_react["default"].createElement(_Close["default"], null));
};

var StartSign = function StartSign(_ref2) {
  var name = _ref2.name,
      handle = _ref2.handle,
      alert = _ref2.alert,
      setName = _ref2.setName,
      setHandle = _ref2.setHandle,
      handleFormSubmit = _ref2.handleFormSubmit,
      buttonStyle = _ref2.buttonStyle;
  return /*#__PURE__*/_react["default"].createElement(_Stack["default"], {
    spacing: 2
  }, /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 36,
      fontWeight: 'bold'
    }
  }, "Leave your ", /*#__PURE__*/_react["default"].createElement("br", null), /*#__PURE__*/_react["default"].createElement("span", {
    style: {
      color: '#257C5E'
    }
  }, "legacy"), "."), /*#__PURE__*/_react["default"].createElement(_material.TextField, {
    label: "name",
    variant: "outlined",
    value: name,
    required: true,
    onInput: function onInput(e) {
      return setName(e.target.value);
    }
  }), /*#__PURE__*/_react["default"].createElement(_material.TextField, {
    label: "twitter handle",
    variant: "outlined",
    value: handle,
    onInput: function onInput(e) {
      return setHandle(e.target.value);
    }
  }), alert && /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 10,
      color: 'red',
      textAlign: 'center'
    }
  }, alert), /*#__PURE__*/_react["default"].createElement(_material.Button, {
    onClick: handleFormSubmit,
    variant: "contained",
    size: "large",
    sx: buttonStyle || styles.button
  }, "Connect wallet to sign"));
};

var ConnectWallet = function ConnectWallet(_ref3) {
  var alert = _ref3.alert,
      handleConnect = _ref3.handleConnect;
  return /*#__PURE__*/_react["default"].createElement(_Stack["default"], {
    spacing: 2
  }, /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 36,
      fontWeight: 'bold'
    }
  }, "Connect your ", /*#__PURE__*/_react["default"].createElement("br", null), /*#__PURE__*/_react["default"].createElement("span", {
    style: {
      color: '#257C5E'
    }
  }, "wallet"), "."), /*#__PURE__*/_react["default"].createElement(_Box["default"], {
    onClick: function onClick() {
      return handleConnect('coinbase');
    },
    sx: {
      cursor: 'pointer',
      borderBottom: '1px solid #eee',
      textAlign: 'center',
      py: 2,
      ':hover': {
        opacity: 0.8
      }
    }
  }, /*#__PURE__*/_react["default"].createElement("img", {
    src: coinbase,
    style: {
      height: 50,
      width: 300
    },
    alt: "login with Coinbase Wallet!"
  })), /*#__PURE__*/_react["default"].createElement(_Box["default"], {
    onClick: function onClick() {
      return handleConnect('metamask');
    },
    sx: {
      cursor: 'pointer',
      textAlign: 'center',
      ':hover': {
        opacity: 0.8
      }
    }
  }, /*#__PURE__*/_react["default"].createElement("img", {
    src: metamask,
    style: {
      height: 100,
      width: 300
    },
    alt: "login with Metamask!"
  })), alert && /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 10,
      color: 'red',
      textAlign: 'center'
    }
  }, alert));
};

var SignMessage = function SignMessage(_ref4) {
  var alert = _ref4.alert,
      handleSign = _ref4.handleSign,
      buttonStyle = _ref4.buttonStyle;
  return /*#__PURE__*/_react["default"].createElement(_Stack["default"], {
    spacing: 2
  }, /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 36,
      fontWeight: 'bold'
    }
  }, "Sign a ", /*#__PURE__*/_react["default"].createElement("span", {
    style: {
      color: '#257C5E'
    }
  }, "message"), " with your wallet."), /*#__PURE__*/_react["default"].createElement(_material.Button, {
    onClick: handleSign,
    variant: "contained",
    size: "large",
    sx: buttonStyle || styles.button
  }, "Sign message"), alert && /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 10,
      color: 'red',
      textAlign: 'center'
    }
  }, alert));
};

var Verify = function Verify(_ref5) {
  var alert = _ref5.alert,
      handleTweet = _ref5.handleTweet,
      handleWithoutVerifying = _ref5.handleWithoutVerifying,
      buttonStyle = _ref5.buttonStyle;
  return /*#__PURE__*/_react["default"].createElement(_Stack["default"], {
    spacing: 2
  }, /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 36,
      fontWeight: 'bold'
    }
  }, "Verify your ", /*#__PURE__*/_react["default"].createElement("span", {
    style: {
      color: '#257C5E'
    }
  }, "signature"), "."), /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 18
    }
  }, "Tweet a message to prove that you control this address. Return here afterwards to complete verification."), /*#__PURE__*/_react["default"].createElement(_material.Button, {
    onClick: handleTweet,
    startIcon: /*#__PURE__*/_react["default"].createElement(_Twitter["default"], null),
    variant: "contained",
    sx: buttonStyle || styles.button
  }, "Post Proof"), alert && /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 10,
      color: 'red',
      textAlign: 'center'
    }
  }, alert), /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 12,
      textAlign: 'center'
    },
    onClick: handleWithoutVerifying
  }, /*#__PURE__*/_react["default"].createElement("a", null, "Continue without verifying")));
};

var LoaderModal = function LoaderModal(_ref6) {
  var title = _ref6.title;
  return /*#__PURE__*/_react["default"].createElement(_Stack["default"], {
    spacing: 2
  }, /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 36,
      fontWeight: 'bold'
    }
  }, title), /*#__PURE__*/_react["default"].createElement(_material.CircularProgress, {
    size: "large"
  }));
};

var VerifyTweet = function VerifyTweet(_ref7) {
  var alert = _ref7.alert,
      handleTwitterVerifyAndSign = _ref7.handleTwitterVerifyAndSign,
      buttonStyle = _ref7.buttonStyle;
  return /*#__PURE__*/_react["default"].createElement(_Stack["default"], {
    spacing: 2
  }, /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 36,
      fontWeight: 'bold'
    }
  }, "Complete ", /*#__PURE__*/_react["default"].createElement("span", {
    style: {
      color: '#257C5E'
    }
  }, "verification"), "."), /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 18
    }
  }, "After sending your tweet, click the button below to complete verification:"), /*#__PURE__*/_react["default"].createElement(_material.Button, {
    onClick: handleTwitterVerifyAndSign,
    startIcon: /*#__PURE__*/_react["default"].createElement(_Twitter["default"], null),
    variant: "contained",
    sx: buttonStyle || styles.button
  }, "Verify Tweet"), alert && /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 10,
      color: 'red',
      textAlign: 'center'
    }
  }, alert));
};

var FinishSign = function FinishSign(_ref8) {
  _objectDestructuringEmpty(_ref8);

  var shareTweet = function shareTweet() {
    var str = "I just built my digital legacy with @legacy_xyz. Build yours at web3legacy.xyz";
    window.open("https://twitter.com/intent/tweet?text=".concat(encodeURI(str)));
  };

  return /*#__PURE__*/_react["default"].createElement(_Stack["default"], {
    spacing: 2
  }, /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 36,
      fontWeight: 'bold'
    }
  }, /*#__PURE__*/_react["default"].createElement("span", {
    style: {
      color: '#257C5E'
    }
  }, "YOU"), " did it!"), /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 20
    }
  }, "Thanks for signing and building your ", /*#__PURE__*/_react["default"].createElement("span", {
    style: {
      color: '#257C5E'
    }
  }, "legacy"), "."), /*#__PURE__*/_react["default"].createElement(_material.Button, {
    onClick: shareTweet,
    startIcon: /*#__PURE__*/_react["default"].createElement(_Twitter["default"], null),
    variant: "contained",
    sx: buttonStyle || styles.button
  }, "Share"));
};

function SignModal(props) {
  var projectId = props.projectId,
      isModalVisible = props.isModalVisible,
      handleClose = props.handleClose,
      handleLoginClick = props.handleLoginClick,
      signFromWallet = props.signFromWallet,
      account = props.account,
      active = props.active,
      buttonStyle = props.buttonStyle,
      modalStyle = props.modalStyle;

  var _useState = (0, _react.useState)(START_SIGN),
      _useState2 = _slicedToArray(_useState, 2),
      state = _useState2[0],
      setState = _useState2[1];

  var _useState3 = (0, _react.useState)(''),
      _useState4 = _slicedToArray(_useState3, 2),
      name = _useState4[0],
      setName = _useState4[1];

  var _useState5 = (0, _react.useState)(''),
      _useState6 = _slicedToArray(_useState5, 2),
      handle = _useState6[0],
      setHandle = _useState6[1];

  var _useState7 = (0, _react.useState)(),
      _useState8 = _slicedToArray(_useState7, 2),
      alert = _useState8[0],
      setAlert = _useState8[1];

  var _useState9 = (0, _react.useState)(false),
      _useState10 = _slicedToArray(_useState9, 2),
      signature = _useState10[0],
      setSignature = _useState10[1];

  var handleFormSubmit = function handleFormSubmit() {
    if (name) {
      setAlert(''); // make sure there are no white spaces

      setName(name.trim());
      setHandle(handle.trim());
      setState(CONNECT_WALLET);
    } else {
      setAlert('Name is required.');
    }
  }; // connects to wallet


  var handleConnect = function handleConnect(provider) {
    handleLoginClick(provider).then(function (res) {
      setAlert('');
      setState(SIGN_MESSAGE);
    })["catch"](function (err) {
      setAlert('An error occured. Please try connecting your wallet again.');
    });
  };

  var handleSign = /*#__PURE__*/function () {
    var _ref9 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              if (active) {
                _context.next = 4;
                break;
              }

              setAlert('Please connect your wallet first.');
              setState(CONNECT_WALLET);
              return _context.abrupt("return");

            case 4:
              signFromWallet(account, name, handle).then(function (sig) {
                setSignature(sig);
                setAlert('');
                (0, _utils.checkIfVerifiedHandle)(handle, sig).then(function (verified) {
                  if (verified) {
                    handleWithoutVerifying();
                  } else {
                    setState(VERIFY);
                  }
                });
              })["catch"](function (err) {
                setAlert('An error occurred. Please try signing again.');
              });

            case 5:
            case "end":
              return _context.stop();
          }
        }
      }, _callee);
    }));

    return function handleSign() {
      return _ref9.apply(this, arguments);
    };
  }();

  var generateTweet = function generateTweet() {
    var str = "I'm building my digital legacy today. Verifying for @legacy_xyz signature:".concat(signature);
    window.open("https://twitter.com/intent/tweet?text=".concat(encodeURI(str)));
  };

  var handleTweet = function handleTweet() {
    generateTweet();
    setState(VERIFY_TWEET);
  };

  var handleTwitterVerifyAndSign = function handleTwitterVerifyAndSign() {
    setState(VERIFYING);
    (0, _utils.verify)(signature, handle).then(function () {
      // verifying that they signed
      (0, _utils.sign)(projectId, name, account, handle, signature).then(function (result) {
        setAlert('');
        setState(FINISH_SIGN);
      })["catch"](function (err) {
        setAlert("An error occurred with signing to the blockchain.");
        setState(VERIFY_TWEET);
      });
    })["catch"](function (err) {
      setAlert("An error occurred. Did you tweet a message?");
      setState(VERIFY);
    });
  };

  var handleWithoutVerifying = function handleWithoutVerifying() {
    setState(SIGNING);
    (0, _utils.sign)(projectId, name, account, handle, signature).then(function (result) {
      setAlert('');
      setState(FINISH_SIGN);
    })["catch"](function (err) {
      setAlert("An error occurred with signing to the blockchain.");
      setState(VERIFY);
    });
  };

  return /*#__PURE__*/_react["default"].createElement(_react["default"].Fragment, null, /*#__PURE__*/_react["default"].createElement(_Modal["default"], {
    open: isModalVisible,
    onClose: handleClose,
    onBackdropClick: handleClose
  }, /*#__PURE__*/_react["default"].createElement(_Box["default"], {
    sx: modalStyle || {
      position: 'absolute',
      top: '50%',
      left: '50%',
      transform: 'translate(-50%, -50%)',
      width: 445,
      background: 'white',
      border: '0px',
      borderRadius: 10,
      boxShadow: 24,
      marginLeft: 'auto',
      marginRight: 'auto',
      p: 5
    }
  }, /*#__PURE__*/_react["default"].createElement(CloseButton, {
    handleClose: handleClose
  }), /*#__PURE__*/_react["default"].createElement(_Stack["default"], {
    sx: {
      pt: 2
    }
  }, state === START_SIGN && /*#__PURE__*/_react["default"].createElement(StartSign, {
    name: name,
    handle: handle,
    alert: alert,
    setName: setName,
    setHandle: setHandle,
    handleFormSubmit: handleFormSubmit,
    buttonStyle: buttonStyle
  }), state === CONNECT_WALLET && /*#__PURE__*/_react["default"].createElement(ConnectWallet, {
    alert: alert,
    handleConnect: handleConnect
  }), state === SIGN_MESSAGE && /*#__PURE__*/_react["default"].createElement(SignMessage, {
    alert: alert,
    handleSign: handleSign,
    buttonStyle: buttonStyle
  }), state === VERIFY && /*#__PURE__*/_react["default"].createElement(Verify, {
    alert: alert,
    handleTweet: handleTweet,
    handleWithoutVerifying: handleWithoutVerifying,
    buttonStyle: buttonStyle
  }), state === VERIFY_TWEET && /*#__PURE__*/_react["default"].createElement(VerifyTweet, {
    alert: alert,
    handleTwitterVerifyAndSign: handleTwitterVerifyAndSign,
    buttonStyle: buttonStyle
  }), state === VERIFYING && /*#__PURE__*/_react["default"].createElement(LoaderModal, {
    title: "Verifying"
  }), state === SIGNING && /*#__PURE__*/_react["default"].createElement(LoaderModal, {
    title: "Signing"
  }), state === FINISH_SIGN && /*#__PURE__*/_react["default"].createElement(FinishSign, null), /*#__PURE__*/_react["default"].createElement(_material.Typography, {
    sx: {
      fontSize: 12,
      textAlign: 'center',
      mt: 3
    }
  }, "\uD83C\uDF31 Check out ", /*#__PURE__*/_react["default"].createElement("a", {
    href: "http://web3legacy.xyz/",
    target: "_blank",
    style: {
      textDecoration: 'none'
    },
    rel: "noreferrer"
  }, /*#__PURE__*/_react["default"].createElement("span", {
    style: {
      color: '#257C5E'
    }
  }, "legacy")), " to learn more")))));
}

var styles = {
  button: {
    color: 'white',
    background: '#000000',
    textTransform: 'none',
    fontSize: 20,
    borderRadius: 3,
    ':hover': {
      background: '#000000',
      opacity: 0.8
    }
  }
};